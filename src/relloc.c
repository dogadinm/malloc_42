...
t_bool	chunk_fusion(t_chunk *chunk, size_t new_size)
{
	...
	if (chunk != NULL)
	{
		chunk_next = (t_chunk*)chunk->next_addr;
		if (chunk_next != NULL && chunk_next->available == TRUE)
		{
			available_space = (chunk->size_data + 
                                sizeof(t_chunk) +
								chunk_next->size_data);
...
t_chunk	*new_allocation(t_chunk *chunk, size_t new_size)
{
	...
	if (new_chunk != NULL)
	{
		copy_data_size(new_chunk->ptr_data, chunk->ptr_data,
										new_chunk->size_data, chunk->size_data);
		free(chunk->ptr_data);
	}
	return (new_chunk);
}
...
t_chunk	*resize_allocation(t_chunk *chunk, size_t new_size)
{
	...
	if (chunk->data_size >= new_size){
		...
		chunk->size_data = new_size;
		new_chunk = chunk;
		if ((t_chunk*)chunk->next_addr != NULL)
			new_chunk->next_addr = ((t_chunk*)chunk->next_addr)->next_addr;
	}
	else{
